...
public class WeatherManager : BackgroundService, IHostedLifecycleService
{
    ...
    private WeatherFxType MapToBaseWeather(WeatherFxType type)
    {
        return (int)type switch
        {
            33 => WeatherFxType.FewClouds,       // LightDrizzleFewClouds -> 16
            34 => WeatherFxType.ScatteredClouds, // LightDrizzleScatteredClouds -> 17
            35 => WeatherFxType.BrokenClouds,    // LightDrizzleBrokenClouds -> 18
            36 => WeatherFxType.ScatteredClouds, // LightRainScatteredClouds -> 17
            37 => WeatherFxType.BrokenClouds,    // LightRainBrokenClouds -> 18
            38 => WeatherFxType.BrokenClouds,    // RainBrokenClouds -> 18
            _ => type
        };
    }
    
    private WeatherType MergeWeather(WeatherType baseType, WeatherType visualType)
    {
        return new WeatherType
        {
            WeatherFxType = visualType.WeatherFxType,

            TemperatureCoefficient = baseType.TemperatureCoefficient,
            RainIntensity = baseType.RainIntensity,
            RainWetness = baseType.RainWetness,
            RainWater = baseType.RainWater,
            Humidity = baseType.Humidity,
    ...
    public void SetCspWeather(WeatherFxType upcoming, int duration)
    {
        ...
        var visualType = _weatherTypeProvider.GetWeatherType(visualTypeEnum);

        var mergedType = MergeWeather(baseType, visualType);

        CurrentWeather.UpcomingType = mergedType;
    ...
    private WeatherFxType MapToVisualWeather(WeatherFxType type)
    {
        return (int)type switch
        {
            33 => WeatherFxType.FewClouds,        // LightDrizzleFewClouds
            34 => WeatherFxType.ScatteredClouds,  // LightDrizzleScatteredClouds
            35 => WeatherFxType.BrokenClouds,     // LightDrizzleBrokenClouds
            36 => WeatherFxType.ScatteredClouds,  // LightRainScatteredClouds
            37 => WeatherFxType.BrokenClouds,     // LightRainBrokenClouds
            38 => WeatherFxType.BrokenClouds,     // RainBrokenClouds
            _ => type
        };
    }
    ...
    public bool SetWeatherConfiguration(int id)
    {
        ...
        var customType = weatherConfiguration.WeatherFxParams.Type;

        // map custom -> base rendering weather (16–18)
        var baseTypeEnum = MapToBaseWeather(customType);
        var baseType = _weatherTypeProvider.GetWeatherType(baseTypeEnum);

        // map custom -> visual override
        var visualTypeEnum = MapToVisualWeather(customType);
        var visualType = _weatherTypeProvider.GetWeatherType(visualTypeEnum);

        var weatherType = MergeWeather(baseType, visualType);
